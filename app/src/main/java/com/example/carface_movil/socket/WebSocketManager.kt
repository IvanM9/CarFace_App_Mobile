package com.example.carface_movil.socket
...
import com.example.carface_movil.guardia.EscanearVehiculo
import com.example.carface_movil.guardia.Solicitud
...
class WebSocketManager:Runnable {
    companion object {
        ...
        var webSocket:dev.icerock.moko.socket.Socket= dev.icerock.moko.socket.Socket(
            ...
            on("get_message") { args ->
                if (context?.let { UTILS.obtieneRol(it) } =="GUARDIA"){
                    ...
                    try {
                        if (choferRepetido(persona)==false){
        ...
        private fun choferRepetido(persona:Persona):Boolean{
